...
$(document).on('click', '#add-product', function() {
    ...
    if (table.rows('.selected').data().length > 0) {
        ...
        <td class="text-end ps-1"> \
            ...
            <p class="mt-1"><select name="whse_uom[]" id="whse_uom_'+idx+'" class="uom uom_select form-select select2"> \
            '+uom+'</select></p> \
        </td> \
        <td class="text-end ps-1"> \
            ...
            <p class="mt-1"><select name="inv_uom[]" id="inv_uom_'+idx+'" class="uom uom_select form-select select2"> \
            '+uom+'</select></p> \
        </td> \
        ...
        success: function (data) {
            if($.isEmptyObject(data.errors)) {
                ...
                $.each(data.errors, function(prefix, val) {
                    ...
                    console.log(prefix);
                    // $('#submit-receive').find('input#'+prefix).addClass('has_error');
