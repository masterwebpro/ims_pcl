...
$(document).on('click','#select-in-daily',function(){
    $("#in-yearly").addClass('d-none');
    $("#in-daily").removeClass('d-none');
    getInboundCount("daily");
});

$(document).on('click','#select-in-yearly',function(){
    $("#in-daily").addClass('d-none');
    $("#in-yearly").removeClass('d-none');
    getInboundCount("yearly");
});
...
$(document).on('change',"#date-range-picker2",function(){
    getOutboundCount("daily");
});

$(document).on('click','#select-out-daily',function(){
    $("#out-yearly").addClass('d-none');
    $("#out-daily").removeClass('d-none');
    getOutboundCount("daily");
});

$(document).on('click','#select-out-yearly',function(){
    $("#out-daily").addClass('d-none');
    $("#out-yearly").removeClass('d-none');
    getOutboundCount("yearly");
});

function getOutboundCount(type){
    $("#out-labels").text(type);
    var date = $("#date-range-picker2").val();
    var year = $("#out-year").val();
    $.ajax({
        url: BASEURL + 'dashboard/getOutboundCount',
        data: {
            type : type,
            date : date,
            year : year,
        ...
        success: function (data) {
            setTimeout(function () {
                $(".outbound-cnt").text(data.tot_trans);
                $(".outbound-qty").text(data.tot_qty);
                outboundChart(data);
            }, 300);
        }
    });
...
function outboundChart(data){
    ...
    if (outboundColors) {
        ...
        if (myOutboundChart) {
            myOutboundChart.destroy();
        }
        myOutboundChart = new ApexCharts(document.querySelector("#outbound_chart"), options);
        myOutboundChart.render();
    }
}
