...
$(document).on('click', '#post-internal-comment', function(){
	var form = $('#messaging');
	...
	$.ajax({
		url: BASEURL + 'settings/post-comment',
		method: "POST",
		data: {
...
$(document).on('click', '#send-email', function(e){
	...
	Swal.fire({
		icon: 'error',
		title: "Sending email is not setup yet",
		showConfirmButton: false,
		timer: 1000
	})
...
$(document).on('click', '#post-red-flag', function(e){
	e.preventDefault();
	var form = $('#messaging');
		...
		success: function (response) {
			if($.isEmptyObject(response.error)) {
				...
				if(response.success == true) {
					...
					$('#red-flag-list').prepend('<div class="alert alert-danger" role="alert" id="rf-'+response.data.id+'><span> <i class="bx bxs-flag-alt"></i> RED FLAG : '+ comment+' </span><span class="float-end"><a href="#" id="red-flag-resolved" data-trans_type="'+response.data.type+'" data-control="'+response.data.control+'" data-id="'+response.data.id+'">Remark as resolve</a></span></div>')
...
$(document).on('click', '#red-flag-resolved', function(e){
	e.preventDefault();
	var id = $(this).data('id');
	...
	Swal.fire({
		title: 'Are you sure?',
		text: "You want to resolved this red flag?",
		icon: 'warning',
