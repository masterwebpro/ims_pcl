...
<head>
  ...
  <title>Transaction Summary | Vehicle Order</title>
...
<body>
    <div class="main">
        <table width="100%">
            <tr>
                <td width="55%" colspan="2" valign="top">
                    <div class="fs-3">
                        <img src="<?=URL::to('/assets/images/pcl_logo.png') ?>" height="60px" height="50px">
                    </div>
                </td>
            </tr>
        </table> 
        ...
        <table width="100%" class="small border">
            <tr class="bold">
                <th class="fw-medium text-center" width="80px;">Date Received</th>
                ...
                <th class="fw-medium text-center" width="65px;">Whse / UOM</th>
                <th class="fw-medium text-center" width="65px;">Inv / UOM</th>
            </tr>
            <? foreach($result as $rec): ?>
            <tr>
                <td>{{date('M d, Y', strtotime($rec->date_received)) }}</td>
                <td>{{$rec->rcv_no}}</td>
                <td>{{$rec->po_num}}</td>
                <td width='100px;'>{{$rec->product_code}}</td>
                <td class="left">{{$rec->product_name}}</td>
                <td class="center">{{$rec->item_type}}</td>
                <td class="right">{{$rec->whse_qty}}  / {{$rec->uw_code}}</td>
                <td class="right">{{$rec->whse_qty}} / {{$rec->ui_code}}</td>
            </tr>
            <? endforeach;?>
        </table> 
    </div>
