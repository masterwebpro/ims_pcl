            ...
            <div class="card-header border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0">
                        <a href="{{ URL::to('maintenance/client/create') }}"class="btn btn-danger add-po" ><i class="ri-add-line align-bottom me-1"></i> Create client</a>
                    </div>
                </div>
            </div>
                        ...
                        <tbody class="list form-check-all">
                            <? if($client->total() > 0 ) : ?>
                                <? foreach($client as $cli) :?>
                                    <tr>
                                        <td class="project_name">{{ $cli->client_code}}</td>
                                        <td class="id">{{ $cli->client_name}}</td>
                                        <td>{{ $cli->client_type ." - ". $type[$cli->client_type]}}</td>
                                        <td>{{ $cli->tin}}</td>
                                        <td>{{ $cli->address_1}}</td>
                                        <td>{{ $cli->address_1}}</td>
                                        <td>{{ $cli->city}}</td>
                                        ...
                                        <td>{{ $cli->country}}</td>
                                        <td>{{ $cli->zipcode}}</td>
                                        <td>{{ $cli->contact_person}}</td>
                                        <td>{{ $cli->email_address}}</td>
                                        <td>{{ $cli->phone_no}}</td>
                                        <td>
                                            <div class="form-check form-switch form-switch-custom form-switch-primary">
                                                <input class="form-check-input" type="checkbox" role="switch" id="SwitchCheck9" name="is_vatable"  {{ (($cli->is_vatable == 1) ? "checked" : "") }} disabled>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="form-check form-switch form-switch-custom form-switch-primary">
                                                <input class="form-check-input" type="checkbox" role="switch" id="SwitchCheck9" name="is_enabled"  {{ (($cli->is_enabled == 1) ? "checked" : "") }} disabled>
                                            </div>
                                        </td>
                                        <td class="created_at">{{ date('M d, Y', strtotime($cli->created_at)) }}</td>
                                        <td class="action">
                                            <div class="hstack gap-3 fs-12">
                                                <a href="{{ URL::to('maintenance/client') }}/<?=_encode($cli->id)?>" data-id="{{$cli->id}}" class="link-info text-info d-inline-block"><i class="ri-eye-fill align-bottom me-1"></i> View</a>
                                                <a href="{{ URL::to('maintenance/client') }}/<?=_encode($cli->id);?>/edit" data-id="{{$cli->id}} " class="link-info edit-po"><i class="ri-pencil-fill align-bottom me-1"></i> Edit </a> </div>
                                        </td>
                                    </tr>
                                <? endforeach; ?>
                            <? else :?>
